...
def test_meeting_schedule_modal_exists() -> None:
    assert issubclass(_MeetingScheduleModal, discord.ui.Modal)


@pytest.mark.asyncio
async def test_step3_embed_shows_meeting_schedule(sheets: MagicMock, cache: MagicMock) -> None:
    orig_interaction = make_interaction()
    view = SetupWizardView(sheets, cache, orig_interaction)
    view.cache.config.meeting_schedule = "every wednesday"
...
@pytest.mark.asyncio
async def test_step3_embed_shows_not_set_when_no_schedule(
    sheets: MagicMock, cache: MagicMock
) -> None:
    orig_interaction = make_interaction()
    view = SetupWizardView(sheets, cache, orig_interaction)
    view.cache.config.meeting_schedule = None
    embed = view._build_schedule_embed()
    field_values = [f.value for f in embed.fields]
...
@pytest.mark.asyncio
async def test_step3_embed_unset_does_not_mention_all_dates_shown(
    sheets: MagicMock, cache: MagicMock
) -> None:
    orig_interaction = make_interaction()
    view = SetupWizardView(sheets, cache, orig_interaction)
    ...
    embed = view._build_schedule_embed()
    field_values = [f.value for f in embed.fields]
    assert not any("all dates shown" in v.lower() for v in field_values)


@pytest.mark.asyncio
async def test_meeting_schedule_modal_valid_writes_and_refreshes(
    sheets: MagicMock, cache: MagicMock
) -> None:
    orig_interaction = make_interaction()
    view = SetupWizardView(sheets, cache, orig_interaction)
    modal = _MeetingScheduleModal(view)
    modal.schedule._value = "every wednesday"
    interaction = make_interaction()
    await modal.on_submit(interaction)
    sheets.update_configuration.assert_called_once_with(
        "meeting_schedule", "every wednesday", type_="string"
    )
    cache.refresh.assert_awaited_once_with(force=True)


@pytest.mark.asyncio
async def test_meeting_schedule_modal_empty_clears(
    sheets: MagicMock, cache: MagicMock
) -> None:
    orig_interaction = make_interaction()
    view = SetupWizardView(sheets, cache, orig_interaction)
    modal = _MeetingScheduleModal(view)
    modal.schedule._value = ""
    interaction = make_interaction()
    await modal.on_submit(interaction)
    sheets.update_configuration.assert_called_once_with(
        "meeting_schedule", "", type_="string"
    )
    cache.refresh.assert_awaited_once_with(force=True)
