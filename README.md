...
to set `host_role_ids`, `admin_role_ids` (JSON arrays of Discord role IDs),
...
`/setup` Step 3 or `/config key:meeting_schedule action:set value:...` (empty
value clears it).
...
`/config key:<name> action:set value:` with no value.
...
- `/config` — 🤫 (owner) view all settings; `/config key:<key> action:set value:<v>` to change one, `/config key:admin|host action:add|remove value:@role` to manage role buckets
